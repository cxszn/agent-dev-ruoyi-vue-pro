...
本页保留当前源码的核心流程；从 `yudao-skills_v1.1.0` 合并的场景与字段资料已按当前基线修正，见[来源说明](import-provenance.md)。
...
5. 预览或下载到待审区域，检查 Controller/VO/Service/DO/Mapper、前端API与页面、SQL与错误码，并按[新增方法注释约定](../../../references/method-comments.md)逐一检查生成方法的注释。按文件合并，保留已有业务分支。
6. 生成菜单与按钮后，验证权限码、角色授权、接口 `@PreAuthorize`、登录菜单及前端入口。应用SQL前核对目标库与已有ID；源代码生成不自动授权修改业务数据库。

## 从需求进入生成流程

先形成最小上下文：后端与相关前端根目录、实际 Java 包、Boot/Cloud 布局、目标模块、表与实体名、数据库方言/目标环境、前端变体、管理后台/用户 App 场景、是否需要查询系统字典。读取工作区与项目配置能确定的事项直接确定，保留用户已指定的目录和选择。不要默认把所有业务放进 system 模块。

| 场景 | 起点与完成条件 |
| --- | --- |
| 已有表 | 读取真实 DDL、迁移或获授权的只读表结构；核对主键、全部列、默认值、可空性、索引和审计/租户字段。只有截图、表名或 DO 时说明结构证据缺口，不声称已查数据库。 |
| 自然语言新功能 | 将需求整理为实体、字段、约束、关系和页面范围；按[字段契约](contracts.md)给出推导依据。产出 DDL 草稿及代码；调用数据库驱动生成器前，表必须已在指定数据源存在，建表执行另核授权。 |
| 已生成模块字段调整 | 进入下面的增量流程，按现有实现修改，不重新生成并覆盖整个模块。 |
| 生成器维护 | 按下面的同步/模板流程验证影响矩阵，不把业务字段修改升级为全局模板变更。 |

批量模块按依赖顺序推进：先确定被引用的字典与主数据，再处理独立实体，最后处理主子关系和菜单。已有表名、状态或“分类”等词仅提供线索；树表须有实际父键语义，订单明细须有真实的联合保存需求。

开发前给出简短摘要：字段/关系、采用的模板、涉及文件、迁移与菜单计划、待决事项。用户已授权的可逆实现直接继续；有实质歧义或未授权的外部写入时再请求必要决定。读取与验证可并行，依赖关系或同文件编辑按顺序执行。

## 已生成模块的增量字段修改

1. 由表名/实体名定位当前 DDL、DO、Mapper/XML、Service、Controller/VO、转换器、API 类型及实际页面；按字段引用扩展到字典、导入导出和相关测试。完成条件是每个字段读写入口都有去向。
2. 记录旧字段契约与新契约：名称/类型、精度或长度、可空与默认值、查询条件、表单控件、校验、字典值、返回字段。新增字段在旧记录上的值须明确；删除/改类型需核对兼容调用方和数据损失。
3. 先写符合项目迁移约定的 SQL 草稿，再同步必要代码。只改实际受影响层；纯展示字段不强加数据库列，数据库新增列也不自动开放客户端写权限。已有业务逻辑、权限和用户修改必须保留。
4. 如同时维护生成器元数据，保存旧配置，核对数据库结构已处于哪个版本，再选择手动修改元数据或获授权的 `sync-from-db`；同步风险见下一节。
5. 验证新建、编辑已有记录、查询与显示，以及本次涉及的导出/移动端路径。交付字段前后差异、真实修改文件、SQL 是否执行、元数据是否同步和未验证项。

SQL 文件名与落点遵循当前项目；来源中的日期序号与 `V...__...sql` 两种格式并不代表项目安装了迁移执行器。创建同名文件前检查已有内容，启动后端也不代表 SQL 自动执行。
