...
| `yudao_change_reviewer` | 固定 diff、原任务验收和测试结果 | 按严重度排序的正确性/权限/数据风险与证据 | 只读；新增方法注释按用户明确验收约定检查，其它个人风格偏好不作为缺陷。 |
...
功能实现和变更审查共同遵守[新增方法注释约定](method-comments.md)，逐一检查本次新增方法及生成产物中的方法。
